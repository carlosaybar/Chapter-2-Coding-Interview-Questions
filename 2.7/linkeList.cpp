...
void intersect(linkedList* list1 , linkedList* list2)
{
    ...
    while (list1->head != NULL && list2->head != NULL) //loops until both lists are empty
    {
        if (list1->data == list2->data) //if the values at the same nodes for both lists are equal
        {
            intersect1 = list1->data; //stores the intersect from list1
            intersect2 = list2->data; //stores the intersect from list2 
